...
def get_score():
    github_service = GitHubService()
    w = DataHandler()
    data = w.read_all_data()
    redis = RedisTool()
    for user in data:
        location, reliability = github_service.get_location(user['location'])
        repos_score = github_service.get_user_repos_contributions(user['login'])
        score = repos_score + user['followers']
        processed_data = {
            'login': user['login'],
            'name': user['name'],
            'location': location,
        ...
        redis.set_value(user['login'], processed_data)
